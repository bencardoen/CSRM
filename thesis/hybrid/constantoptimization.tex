...
\[
f(x, y, z) = \tan(y) * \sqrt{x/x}
\]
In this reduced example the node $\sqrt{x/x}$ is simply $\sqrt{1}$. Outside of this example, detecting such cases is non trivial. There is clear benefit to do so, despite their low occurrence, if x's domain includes 0 this valid tree is never generated because it leads to division by zero in a single datapoint. One way to solve this is to use mathematical software to simplify the expressions, and then convert them back to tree representations. The cost of doing this should outweigh the benefit of applying it. It is not clear how frequent such cases are in real world data. % link future work
